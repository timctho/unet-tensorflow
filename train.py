...
ids_train_split, ids_valid_split = train_test_split(ids_train, test_size=0.01, random_state=42)
...
model = UNet(1024, is_training=True)
...
with tf.Session(config=config) as sess:
    ...
    while True:
        ...
        _, loss_np, gs_np, dice_acc_np, summary_np = sess.run([train_op, model.loss, model.gs, model.dice_acc,
                                                               model.merged_summary],
                                                              feed_dict={model.input: x_train,
                                                                         model.gt_mask: y_train,
                                                                         model.loss_w: w_train})
        tmp_np = model.loss_out.eval()
        print(np.amax(tmp_np), np.amin(tmp_np))
        tf_writer.add_summary(summary_np, global_step=gs_np)
        print('step {:>6}, loss {:>1.5f}, dice {:>1.5f}'.format(gs_np, loss_np, dice_acc_np))

        if (gs_np + 1) % 5000 == 0:
            saver.save(sess, save_path='model/', global_step=gs_np)
